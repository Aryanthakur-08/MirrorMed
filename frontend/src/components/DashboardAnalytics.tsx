...
export default function DashboardAnalytics({ patients = [], doctors = [], module }: DashboardAnalyticsProps) {
    if (module === "cohort") {
        ...
        // 2. Age Distribution (Dynamic bins — supports pediatric through geriatric)
        const hasPediatric = patients.some(p => p.age < 18);
        const allBins: Record<string, number> = {
            ...(hasPediatric ? { '0-1': 0, '2-5': 0, '6-12': 0, '13-17': 0 } : {}),
            '18-30': 0, '31-40': 0, '41-50': 0, '51-60': 0, '61-70': 0, '71+': 0
        };
        patients.forEach(p => {
            if (p.age <= 1) allBins['0-1'] = (allBins['0-1'] || 0) + 1;
            else if (p.age <= 5) allBins['2-5'] = (allBins['2-5'] || 0) + 1;
            else if (p.age <= 12) allBins['6-12'] = (allBins['6-12'] || 0) + 1;
            else if (p.age <= 17) allBins['13-17'] = (allBins['13-17'] || 0) + 1;
            else if (p.age <= 30) allBins['18-30']++;
            else if (p.age <= 40) allBins['31-40']++;
            else if (p.age <= 50) allBins['41-50']++;
            else if (p.age <= 60) allBins['51-60']++;
            else if (p.age <= 70) allBins['61-70']++;
            else allBins['71+']++;
        });
        // Only show bins that have patients or are in the relevant range
        const ageData = Object.entries(allBins)
